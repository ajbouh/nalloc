...
class Nalloc::Driver::VirtualBox < Nalloc::Driver
  ...
  def start_allocating_node(cluster_id, name, spec, vbox_info)
    ...
    return lambda do
      ...
      nat_ssh_config = Nalloc.trace(operation: 'waiting for ssh') do
        first_nat_ssh_connection(name, user, vbox_info[:template_key_path],
            ssh_nat_port)
      end

      Nalloc.trace(operation: 'setting public ip and authorizing ssh key') do
        command = <<-EOS
sudo sh -c '(echo \"#NALLOC-BEGIN
