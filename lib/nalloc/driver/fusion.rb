...
class Nalloc::Driver::Fusion < Nalloc::Driver
  ...
  def start_allocating_nodes(cluster_id, specs)
    ...
    specs.each do |name, spec|
      nodes[name] =
        start_allocating_node(cluster_id, name, spec,
    ...
    adapter_pool.release(adapter) if adapter
  ...
  #
  # Side effects:
  # - Creates a directory to house the vm if it does not already exist. This
  #   directory lives at VM_STORE_PATH/*cluster_id*
  #
  # @param  [String]  cluster_id  Globally unique cluster id.
  ...
  def start_allocating_node(cluster_id, name, specs, props)
